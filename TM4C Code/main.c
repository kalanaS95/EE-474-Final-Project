...
void AndroidToTivaController(void)
{
    ...
    //---- ultra sonic
        ...
        GPIO_PORTF_DATA_R |= 0x08;
        //timer_waitMicros(10);       // Waiting for 10us
...
void Setup_GPIOF(void)
{
    ...
    // 1. Enable Clock to the GPIO Modules (SYSCTL->RCGCGPIO) GPIO_F
    SYSCTL_RCGCGPIO_R |= (1<<5);

    // allow time for clock to stabilize (SYSCTL->PRGPIO)
    //while((SYSCTL_PRGPIO_R & 0x5) != 0x1){};
    delay++;
    // 2. Unlock PD7 and PF0 on TM4C123G; or PD7 on TM4C1294 (GPIO->LOCK and GPIO->CR) not used
    //GPIO_PORTF_LOCK_R = 0x4C4F434B;

    // 3. GPIO Analog Mode Select (GPIOAMSEL) all GPIO_PORTF are not analog
    GPIO_PORTF_AMSEL_R = 0x00;

    // 4. GPIO Port COntrol (GPIOPCTL) set bit 8,9,10 in PMC2 of GPIO_PORTF PCTL
    GPIO_PORTF_PCTL_R |= 0x0700;

    // 5. Clear AFSEL bits to 0 to select regular I/O (NEED TO SET AF FUNCTION FOR PF2)
    GPIO_PORTF_AFSEL_R |= 0x4;

    // 6. GPIO Pin Direction (GPIODIR) 0 for input, 1 for output (PF2 input, PF3 output)
...
void Setup_TIMER1(void)
{
    ...
    // trying edge-timer mode

    // 1 . Enable Clock for TIMER Module
    //SYSCTL->RCGCTIMER |= (_______);
    ...
    // 3. Configure TIMER
    TIMER1_CFG_R = 0x04;         // Split into two 16-bit timers

    // 4. Configure Timer n Mode: GPTMTAMR
    // capture mode (0x3) edge-time mode (0x4)
    TIMER1_TAMR_R = 0x7;
...
uint32_t measureD(void)
{
    ...
    //   corresponding bit on GPTMICR (TIMER1->ICR) register (write 1 to bit 2)
    TIMER1_ICR_R |= 0x4;
    ...
    //6. Set the lowEdge from GPTMTAR (TIMER1->TAR) registers
